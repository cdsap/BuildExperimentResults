...
class ChartGenerator(val report: Report) {
    ...
    fun generateCharts(variants: Map<String, List<BuildWithResourceUsage>>, header: Header): String {
        ...
        val containsKotlinProcess = hasKotlinProcess(variants)
        val containsGradleProcess = hasGradleProcess(variants)
        val containsBuildReports = hasBuildReports(variants)
        val uniqueTotalCollections = getUniqueTotalCollections(variants)

        return """
            <div class="charts-grid">
                ${generateBasicCharts(mostExpensiveTaskPath)}
                ${generateProcessCharts(variants, containsKotlinProcess, containsGradleProcess)}
